...
- **Navigation**: Deep link shape is `/script/:sessionId/:scriptId`. If only the session is specified, the UI resolves to the **latest** snapshot for that session. The top-level Script list distinguishes generated scripts from unfinished chats; unfinished chats route back to Chat instead of opening an empty script workspace.
...
- **Voice Studio**: `/voice-studio` is the global reusable voice library, and `/voice-studio/:sessionId/:scriptId` remains the management entry point for creating, previewing, and deleting voice profiles. It does not generate or manage final podcast audio.
...
- **Script handoff**: Selecting a profile from the Script Workbench voice dropdown writes the current script's `artifact.voice_reference` with `source: "voice_profile"` and `voice_profile_id`. Script Workbench refreshes that profile reference before local MLX final rendering so older MP4/M4A uploads are prepared as a WAV reference when the platform converter is available.
- **UI**: The script route focuses on editing and final-audio production/review; the right-side Voice Workspace owns inline voice-profile selection, while Voice Studio owns reference-audio preview and profile creation. The Script list also exposes lightweight cleanup: generated rows can move the latest script snapshot to trash, while unfinished rows can move the chat/session draft to trash.

## Audio-only MP4 / M4A scope

The current app can request provider/runtime output formats and serve common audio suffixes, but it does not transcode generated WAV to AAC/M4A/MP4 itself. `.mp4` means audio-only container support when the selected provider/runtime produces a valid file. User-uploaded profile samples may be converted into an app-managed WAV reference for local MLX compatibility when `ffmpeg` or `afconvert` is available. True video MP4 and guaranteed generated WAV → AAC conversion require a separate packaging decision.
