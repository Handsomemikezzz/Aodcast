...
Voice profiles are the canonical voice source for profile-first rendering. The three built-in profiles are packaged app assets under `services/python-core/app/assets/voice-profiles/` and contain the English reference line “Hello, welcome to use Aodcast. What shall we talk about today?”. User-saved profiles are stored under `.local-data/voice-profiles/user-profiles.json`, with profile audio copied into `.local-data/exports/_voice_profiles` so it can be served through the normal artifact audio route. Creating a user profile requires a name, managed reference audio path, and the exact reference text spoken in that audio.
