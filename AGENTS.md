...
- 2026-04-30: Voice profiles are reusable Voice Studio assets. Built-in/user profiles are managed by `VoiceProfileStore`; user profile audio is copied into `.local-data/exports/_voice_profiles` and selected profiles write script-scoped `artifact.voice_reference` with `source: voice_profile` plus `voice_profile_id`. Do not treat profile audio as a transient preview; deleting a profile must clear matching script references, but deleting a preview should not remove saved profiles.
