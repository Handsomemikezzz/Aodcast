...
class OpenAICompatibleTTSProvider:
    ...
    def synthesize(self, request: TTSGenerationRequest) -> TTSGenerationResponse:
        ...
        if not self.config.api_key_env:
            raise ValueError("OpenAI-compatible TTS provider requires an api_key_env.")

        api_key = os.getenv(self.config.api_key_env, "")
        if not api_key:
            raise ValueError(
                f"Environment variable '{self.config.api_key_env}' is required for the configured TTS provider."
            )
